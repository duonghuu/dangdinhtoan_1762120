...
if(!empty($_POST["rp2val"])){
    if($_SESSION['rp2token'] == $_POST['rp2token']){ // refresh page
      ...
      if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recaptchaResponse'])) {
          ...
          // Take action based on the score returned:
          if (!$recaptcha->success == true) { 
            transfer(_xindungspamwebsitechungtoi, "contact.html");
          }
            $datapost = $_POST["fpu"];
            ...
            $data['chude'.$lang] = "Liên hệ báo giá ".date('d/m/Y h:i:s',time());
            $data['hienthi'] = 1;
            $data['stt'] = 1;
            $data['type'] = 'datban';
           // $data['noidung'] = '<div><h2>Nội dung liên hệ</h2>
      ...
      if($_SESSION['rp3token'] == $_POST['rp3token']){ // refresh page
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['recaptcha2'])) {
            ...
            if ($recaptcha->score >= 0.5 and $recaptcha->success == true) { 
              ...
              $data['chude'.$lang] = "Tư vấn ".date('d/m/Y h:i:s',time());
              $data['hienthi'] = 0;
              $data['stt'] = 1;
              $data['type'] = 'datban';
              ...
              $body .= '
                ...
                <tr>
                  <th colspan="2">Tư vấn từ website '.$company["ten"].'</th>
                </tr>
                <tr>
                  <th colspan="2">&nbsp;</th>
                </tr>
